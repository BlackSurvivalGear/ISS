...
const openShiftManager=async id=>{
  ...
  const candidates=shiftMembers.filter(m=>m.status==="active"&&!canManageShifts(m.role)&&!assigned.has(m.uid));
  ...
  byId("manageShiftAvailability").innerHTML=(available.length?'<small class="available-count">'+available.length+' available</small>':'<small>No available officers for this shift.</small>')+unavailable.map(({m,conflict})=>'<div class="officer-unavailable"><span>'+escapeHtml(officerName(m))+(m.phone?' · <a href="tel:'+escapeHtml(m.phone.replace(/[^+\\d]/g,""))+'">'+escapeHtml(m.phone)+'</a>':'')+'</span><small>Unavailable · '+escapeHtml(conflict.siteName||"Shift")+' '+escapeHtml(conflict.startTime||"")+'–'+escapeHtml(conflict.endTime||"")+'</small></div>').join("");
  const draw=()=>{byId("manageShiftAssignments").innerHTML=(shift.assignments||[]).length?(shift.assignments||[]).map(a=>'<span>'+escapeHtml(a.name)+(a.phone?' · <a href="tel:"+escapeHtml(a.phone.replace(/[^+\\d]/g,""))+"'>"+escapeHtml(a.phone)+"</a>":'')+' <button type="button" class="remove-assignment" data-uid="'+a.uid+'">Remove</button></span>').join(""):'<small>No officers assigned.</small>';document.querySelectorAll(".remove-assignment").forEach(x=>x.onclick=()=>{shift.assignments=(shift.assignments||[]).filter(a=>a.uid!==x.dataset.uid);draw()})};draw();
  byId("manageShiftOfficer").onchange=e=>{const m=shiftMembers.find(x=>x.uid===e.target.value);if(!m)return;shift.assignments=shift.assignments||[];if(shift.assignments.length>=Number(form.elements.positions.value||1)){alert("This shift is full.");e.target.value="";return}shift.assignments.push({uid:m.uid,name:[m.firstName,m.lastName].filter(Boolean).join(" ")||m.email,email:m.email||"",phone:m.phone||"",bookedAt:new Date().toISOString(),assignedBy:auth.currentUser.uid});e.target.value="";draw()};
