...
const renderShifts=()=>{
  ...
  byId("shiftList").innerHTML=visible.length?visible.map(s=>{const mine=(s.assignments||[]).some(a=>a.uid===uid),open=Math.max(0,Number(s.positions||1)-(s.assignments||[]).length);const action=mine?'<span class="admin-status">BOOKED</span>':manager?'<span class="admin-status">MANAGE</span>':open>0?'<button class="secondary claim-shift" data-shift="'+s.id+'">Book Shift</button>':'<span class="admin-status">FULL</span>';return '<article class="shift-card"><div><small>'+escapeHtml(s.date||"")+'</small><h3>'+escapeHtml(s.siteName||"Site")+'</h3><span>'+escapeHtml(s.startTime||"")+'–'+escapeHtml(s.endTime||"")+' · '+escapeHtml(s.requiredRole||"Officer")+'</span></div><div><strong>'+open+'</strong><small>OPEN</small></div><div class="shift-assignees">'+(s.assignments||[]).map(a=>'<span>'+escapeHtml(a.name)+'</span>').join("")+'</div>'+action+'</article>'}).join(""):'<div class="empty-sites">No shifts scheduled for this month.</div>';
