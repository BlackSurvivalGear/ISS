...
byId("shiftEditorForm").addEventListener("submit",async e=>{e.preventDefault();const d=formData(e.currentTarget),site=byId("shiftSite").selectedOptions[0],button=e.currentTarget.querySelector('[type="submit"]');const repeatLabel=d.recurrence&&d.recurrence!=="none"?" · "+d.recurrence+" until "+d.repeatUntil:"";if(!await issConfirm(d.recurrence&&d.recurrence!=="none"?"Create recurring shifts":"Create shift",(site?.textContent||"Site")+" · "+d.date+" · "+d.startTime+"–"+d.endTime+" · "+d.positions+" position(s)"+repeatLabel))return;button.disabled=true;try{const result=await createShift(currentDashboard.companyId,{...d,siteName:site?.textContent||""});message(byId("shiftEditorMessage"),result.count>1?result.count+" recurring shifts created successfully.":"Shift created successfully.");shiftData=await listShifts(currentDashboard.companyId);renderShifts();setTimeout(()=>{e.currentTarget.reset();byId("shiftEditor").hidden=true},650)}catch(err){message(byId("shiftEditorMessage"),err.message,true)}finally{button.disabled=false}});
