...
const renderWeeklyCalendar=()=>{
 ...
 const headers=week.map(d=>{const iso=localIso(d),today=iso===todayIso;return '<div class="week-day-head'+(today?" week-today":"")+'"><strong>'+d.toLocaleDateString(undefined,{weekday:"short"})+(today?' <small class="today-label">TODAY</small>':"")+'</strong><span>'+d.getDate()+'</span></div>'}).join("");
 const row=period=>'<div class="week-row-label"><strong>'+(period==="day"?"DAY":"NIGHT")+'</strong><small>'+(period==="day"?"Day shifts":"Night shifts")+'</small></div>'+week.map(d=>{const iso=localIso(d),items=shiftData.filter(s=>s.date===iso&&shiftPeriod(s)===period).sort((a,b)=>String(a.startTime).localeCompare(String(b.startTime)));return '<div class="week-cell'+(iso===todayIso?" week-today":"")+'" data-date="'+iso+'">'+(items.length?items.map(card).join(""):'<span class="week-empty">No shift</span>')+'</div>'}).join("");
...
let calendarMode="week",calendarFocusDate=new Date();
const simpleCalendarCard=s=>{const assigned=(s.assignments||[]).length,open=Math.max(0,Number(s.positions||1)-assigned),full=open===0;return '<article class="week-shift '+(full?"week-shift-staffed":"week-shift-vacant")+'"><div class="week-shift-head"><strong>'+escapeHtml(s.siteName||"Site")+'</strong><small>'+escapeHtml(s.startTime||"")+'–'+escapeHtml(s.endTime||"")+'</small></div><span class="week-staffing">'+(full?"FULLY STAFFED":open+" OPEN")+'</span><small>'+assigned+'/'+Number(s.positions||1)+' assigned</small></article>'};
const renderDayCalendar=()=>{const d=new Date(calendarFocusDate);d.setHours(12,0,0,0);const iso=localIso(d),items=shiftData.filter(s=>s.date===iso).sort((a,b)=>String(a.startTime).localeCompare(String(b.startTime))),today=iso===localIso(new Date());byId("weekRange").textContent=d.toLocaleDateString(undefined,{weekday:"long",day:"numeric",month:"long",year:"numeric"});byId("calendarSubtitle").textContent="Detailed Day and Night coverage";const group=p=>{const list=items.filter(s=>shiftPeriod(s)===p);return '<section class="day-calendar-section"><h3>'+(p==="day"?"DAY":"NIGHT")+' <small>'+list.length+' shift'+(list.length===1?"":"s")+'</small></h3><div class="day-calendar-list">'+(list.length?list.map(simpleCalendarCard).join(""):'<div class="calendar-empty">No '+p+' shifts</div>')+'</div></section>'};byId("shiftCalendar").innerHTML='<div class="day-calendar'+(today?" calendar-today-panel":"")+'">'+(today?'<span class="today-label">TODAY</span>':"")+group("day")+group("night")+'</div>'};
const renderMonthCalendar=()=>{const f=new Date(calendarFocusDate),y=f.getFullYear(),m=f.getMonth(),first=new Date(y,m,1),last=new Date(y,m+1,0),today=localIso(new Date()),lead=(first.getDay()+6)%7,cells=Array.from({length:lead},()=>'<div class="month-cell month-outside"></div>');for(let n=1;n<=last.getDate();n++){const d=new Date(y,m,n,12),iso=localIso(d),items=shiftData.filter(s=>s.date===iso),open=items.reduce((x,s)=>x+Math.max(0,Number(s.positions||1)-(s.assignments||[]).length),0);cells.push('<button type="button" class="month-cell'+(iso===today?" week-today":"")+'" data-calendar-date="'+iso+'"><span class="month-date">'+n+(iso===today?' <small class="today-label">TODAY</small>':"")+'</span><strong>'+items.length+' shift'+(items.length===1?"":"s")+'</strong><small class="'+(open?"month-open":"month-full")+'">'+(items.length?(open?open+" open":"Fully staffed"):"No shifts")+'</small></button>')}byId("weekRange").textContent=f.toLocaleDateString(undefined,{month:"long",year:"numeric"});byId("calendarSubtitle").textContent="Monthly operations overview";byId("shiftCalendar").innerHTML='<div class="month-calendar"><div class="month-weekdays">'+["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].map(x=>"<strong>"+x+"</strong>").join("")+'</div><div class="month-grid">'+cells.join("")+'</div></div>';document.querySelectorAll("[data-calendar-date]").forEach(btn=>btn.onclick=()=>{calendarFocusDate=new Date(btn.dataset.calendarDate+"T12:00:00");setCalendarMode("day")})};
const renderCalendar=()=>{if(calendarMode==="day")return renderDayCalendar();if(calendarMode==="month")return renderMonthCalendar();calendarWeekStart=mondayFor(calendarFocusDate);byId("calendarSubtitle").textContent="Monday–Sunday duty coverage";renderWeeklyCalendar()};
const setCalendarMode=mode=>{calendarMode=mode;["day","week","month"].forEach(x=>byId("calendar"+x[0].toUpperCase()+x.slice(1)+"Mode").classList.toggle("active",x===mode));renderCalendar()};
let shiftMembers=[];
...
const openCalendarView=async()=>{if(!currentDashboard)return;hideOperationalViews();byId("calendarView").hidden=false;window.scrollTo({top:0});shiftData=await listShifts(currentDashboard.companyId);calendarFocusDate=new Date();calendarMode="week";setCalendarMode("week")};
...
document.querySelectorAll("#shiftEditorForm .native-picker, #shiftManageForm .native-picker").forEach(input=>{input.addEventListener("click",()=>{if(typeof input.showPicker==="function"){try{input.showPicker()}catch{}}});input.addEventListener("keydown",e=>{if((e.key==="Enter"||e.key===" ")&&typeof input.showPicker==="function"){e.preventDefault();try{input.showPicker()}catch{}}})});
const setShiftMonth=date=>{const d=new Date(date.getFullYear(),date.getMonth(),1,12);byId("shiftMonth").value=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`;byId("shiftMonthLabel").textContent=d.toLocaleDateString(undefined,{month:"long",year:"numeric"});renderShifts()};let shiftMonthDate=new Date();setShiftMonth(shiftMonthDate);byId("previousShiftMonth").addEventListener("click",()=>{shiftMonthDate=new Date(shiftMonthDate.getFullYear(),shiftMonthDate.getMonth()-1,1,12);setShiftMonth(shiftMonthDate)});byId("nextShiftMonth").addEventListener("click",()=>{shiftMonthDate=new Date(shiftMonthDate.getFullYear(),shiftMonthDate.getMonth()+1,1,12);setShiftMonth(shiftMonthDate)});
byId("calendarDayMode").addEventListener("click",()=>setCalendarMode("day"));byId("calendarWeekMode").addEventListener("click",()=>setCalendarMode("week"));byId("calendarMonthMode").addEventListener("click",()=>setCalendarMode("month"));
byId("previousWeek").addEventListener("click",()=>{const d=new Date(calendarFocusDate);if(calendarMode==="day")d.setDate(d.getDate()-1);else if(calendarMode==="week")d.setDate(d.getDate()-7);else d.setMonth(d.getMonth()-1);calendarFocusDate=d;renderCalendar()});
byId("nextWeek").addEventListener("click",()=>{const d=new Date(calendarFocusDate);if(calendarMode==="day")d.setDate(d.getDate()+1);else if(calendarMode==="week")d.setDate(d.getDate()+7);else d.setMonth(d.getMonth()+1);calendarFocusDate=d;renderCalendar()});
byId("thisWeek").addEventListener("click",()=>{calendarFocusDate=new Date();renderCalendar()});
...
byId("deleteShift").addEventListener("click",async()=>{const form=byId("shiftManageForm"),id=form.elements.id.value,shift=shiftData.find(s=>s.id===id);if(!shift)return;const scope=shift.seriesId?form.elements.seriesScope.value:"one",scopeLabel=scope==="all"?"the entire recurring series":scope==="future"?"this and all future shifts":"this shift only";if(!await issConfirm("Delete shift",(shift.siteName||"Site")+" · "+(shift.date||"")+" · "+(shift.startTime||"")+"–"+(shift.endTime||"")+" — Delete "+scopeLabel+"? This cannot be undone.",true))return;try{await deleteShift(currentDashboard.companyId,id,scope);shiftData=await listShifts(currentDashboard.companyId);renderShifts();byId("shiftManage").hidden=true;window.scrollTo({top:0})}catch(err){message(byId("shiftManageMessage"),err.message,true)}});
