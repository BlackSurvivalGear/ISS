import { registerCompany, getDashboardData, listSites, saveSite, deleteSite, listInvitations, saveInvitation, setInvitationStatus, deleteTeamMember, acceptInvitation, getPlatformOverview, updateUserProfile, listShifts, createShift, claimShift, updateShift, deleteShift, listCompanyMembers } from "./backend.js";
...
const renderShifts=()=>{
  ...
  const manager=canManageShifts(currentDashboard?.role);
  byId("shiftList").innerHTML=visible.length?visible.map(s=>{const mine=(s.assignments||[]).some(a=>a.uid===uid),open=Math.max(0,Number(s.positions||1)-(s.assignments||[]).length);const action=manager?'<button class="secondary manage-shift" data-shift="'+s.id+'">Manage</button>':mine?'<span class="admin-status">BOOKED</span>':open>0?'<button class="secondary claim-shift" data-shift="'+s.id+'">Book Shift</button>':'<span class="admin-status">FULL</span>';return '<article class="shift-card"><div><small>'+escapeHtml(s.date||"")+'</small><h3>'+escapeHtml(s.siteName||"Site")+'</h3><span>'+escapeHtml(s.startTime||"")+'–'+escapeHtml(s.endTime||"")+' · '+escapeHtml(s.requiredRole||"Officer")+'</span></div><div><strong>'+open+'</strong><small>OPEN</small></div><div class="shift-assignees">'+(s.assignments||[]).map(a=>'<span>'+escapeHtml(a.name)+'</span>').join("")+'</div>'+action+'</article>'}).join(""):'<div class="empty-sites">No shifts scheduled for this month.</div>';
...
const openShiftManager=async id=>{
  ...
  const assigned=new Set((shift.assignments||[]).map(a=>a.uid));const eligible=shiftMembers.filter(m=>m.status==="active"&&!canManageShifts(m.role)&&!assigned.has(m.uid));
  byId("manageShiftOfficer").innerHTML='<option value="">Select officer to assign</option>'+eligible.map(m=>'<option value="'+m.uid+'">'+escapeHtml([m.firstName,m.lastName].filter(Boolean).join(" ")||m.email)+'</option>').join("");
  const draw=()=>{byId("manageShiftAssignments").innerHTML=(shift.assignments||[]).length?(shift.assignments||[]).map(a=>'<span>'+escapeHtml(a.name)+' <button type="button" class="remove-assignment" data-uid="'+a.uid+'">Remove</button></span>').join(""):'<small>No officers assigned.</small>';document.querySelectorAll(".remove-assignment").forEach(x=>x.onclick=()=>{shift.assignments=(shift.assignments||[]).filter(a=>a.uid!==x.dataset.uid);draw()})};draw();
  byId("manageShiftOfficer").onchange=e=>{const m=shiftMembers.find(x=>x.uid===e.target.value);if(!m)return;shift.assignments=shift.assignments||[];if(shift.assignments.length>=Number(form.elements.positions.value||1)){alert("This shift is full.");e.target.value="";return}shift.assignments.push({uid:m.uid,name:[m.firstName,m.lastName].filter(Boolean).join(" ")||m.email,email:m.email||"",bookedAt:new Date().toISOString(),assignedBy:auth.currentUser.uid});e.target.value="";draw()};
...
byId("addShift").addEventListener("click",()=>{byId("shiftEditorMessage").textContent="";byId("shiftEditor").hidden=false});
...
byId("shiftEditorForm").addEventListener("submit",async e=>{e.preventDefault();const d=formData(e.currentTarget),site=byId("shiftSite").selectedOptions[0],button=e.currentTarget.querySelector('[type="submit"]');if(!confirm("Create this shift?\n\n"+(site?.textContent||"Site")+" · "+d.date+" · "+d.startTime+"–"+d.endTime+" · "+d.positions+" position(s)"))return;button.disabled=true;try{await createShift(currentDashboard.companyId,{...d,siteName:site?.textContent||""});message(byId("shiftEditorMessage"),"Shift created successfully.");shiftData=await listShifts(currentDashboard.companyId);renderShifts();setTimeout(()=>{e.currentTarget.reset();byId("shiftEditor").hidden=true},650)}catch(err){message(byId("shiftEditorMessage"),err.message,true)}finally{button.disabled=false}});
byId("deleteShift").addEventListener("click",async()=>{const id=byId("shiftManageForm").elements.id.value,shift=shiftData.find(s=>s.id===id);if(!shift)return;if(!confirm("Delete this shift permanently?\n\n"+(shift.siteName||"Site")+" · "+(shift.date||"")+" · "+(shift.startTime||"")+"–"+(shift.endTime||"")+"\n\nThis cannot be undone."))return;try{await deleteShift(currentDashboard.companyId,id);shiftData=await listShifts(currentDashboard.companyId);renderShifts();byId("shiftManage").hidden=true;window.scrollTo({top:0})}catch(err){message(byId("shiftManageMessage"),err.message,true)}});
byId("shiftManageForm").addEventListener("submit",async e=>{e.preventDefault();const d=formData(e.currentTarget),shift=shiftData.find(s=>s.id===d.id),site=byId("manageShiftSite").selectedOptions[0];if(!shift)return;if(!confirm("Save these shift changes?"))return;try{await updateShift(currentDashboard.companyId,d.id,{...d,siteName:site?.textContent||"",assignments:shift.assignments||[]});message(byId("shiftManageMessage"),"Shift updated successfully.");shiftData=await listShifts(currentDashboard.companyId);renderShifts();setTimeout(()=>byId("shiftManage").hidden=true,500)}catch(err){message(byId("shiftManageMessage"),err.message,true)}});
